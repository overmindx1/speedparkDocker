...
class OrderListRepos {
     ...
     * @return \Illuminate\Database\Eloquent\Collection|OrderList[]
     */
    public function getBuyCountryCountByDateRange($startDate , $endDate , $sellers) {
        $countList = $this->orderList->select(\DB::raw('`shippingCountry` as Country , COUNT(`shippingCountryCode`) as Count'))
                                     ->where('created_at' , '>=' , $startDate)
                                     ->where('created_at' , '<=' , $endDate)
                                     ->whereIn('ebaySeller' , $sellers)
                                     ->groupBy('shippingCountry')->get();
        return $countList;
    }
    ...
    public function insertIpnOrder(array $ipnOrder ) {
        ...
        if($orderStatus['action'] == 'Create') { //看有沒有新單
            ...
            //更新buyer mail  , phone
            $this->updateOrderEmailAndPhone($orderResult);
     ...
     * //更新buyer mail  , phone
     */
    private function updateOrderEmailAndPhone(OrderList $order){
        if($order->ebaySeller != 'send_money') { //如果不是send money
